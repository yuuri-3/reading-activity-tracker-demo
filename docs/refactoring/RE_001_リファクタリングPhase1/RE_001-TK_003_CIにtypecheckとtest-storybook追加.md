...
- [ ] 品質ゲートに落ちた変更は原則デプロイされない（PR の必須チェックとして運用する）
- [ ] 直 push / ブランチ保護の一時解除があり得る運用の場合、デプロイ用ワークフロー側にも `typecheck` を入れ「型エラー等の明確な事故」はデプロイ手前で確実に止まる
- [ ] PR でも `typecheck` と `test-storybook` が実行され、早期に回帰検知できる
- [ ] ブランチ保護で品質ゲートを必須チェックにし、管理者のみ緊急時に一時的な override を許可する
- [ ] `npm run test-storybook` が Storybook test runner として実行される（Vitest の `--project=storybook` ではない）
- [ ] Storybook test runner に必要な Playwright Chromium の導入が CI に含まれている
- [ ] PR の品質ゲートは Secrets を必須にせず完走できる（外部サービス依存はモック/スタブで回避）
- [ ] 外部連携（本物の Firebase/Functions 等）まで含めた保証は本チケットの範囲外であることが明記されている
...
→ 整理チケット: [RE_001-TK_023 `test-storybook` 移行後の Vitest（storybook project）設定整理](RE_001-TK_023_test-storybook移行後のVitest設定整理.md)
...
- 完全 A（主要 Story を外部依存なしで安定稼働）に向けたモック/依存注入の整理
  - チケット: [RE_001-TK_021 Storybook の外部依存をモック化して安定稼働（完全 A）](RE_001-TK_021_Storybook外部依存をモック化して安定稼働（完全A）.md)
- 外部連携まで含めた保証（E2E / エミュレータ / ステージング検証など）の方針決定と導入
  - チケット: [RE_001-TK_022 外部連携まで含めた保証（E2E/エミュレータ/ステージング）の方針決定と導入](RE_001-TK_022_E2E保証（エミュレータ・ステージング含む）の方針決定と導入.md)
