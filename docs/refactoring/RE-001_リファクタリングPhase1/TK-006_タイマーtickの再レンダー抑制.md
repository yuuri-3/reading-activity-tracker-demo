...
- [ ] タイマー表示は **秒単位で自然に更新** される（`mm:ss` が不自然に止まらない）
...
- React DevTools Profiler / Highlight updates を使い、タイマー動作中に Timer と無関係な画面要素が継続的に更新されていないことを確認
- 例: Timer 計測を開始した状態で TimerPage 以外へ遷移しても、画面全体が tick ごとに点滅（更新ハイライト）し続けない
- タイマー画面の表示（秒表示の切り替わり）が不自然に止まらないことを確認
- リロード/復帰時にタイマーが意図した状態（再開/一時停止/リセット）で復元されることを確認
